...
function processVerts(coords, offset, name) {
  ...
  if (typeof(polys) != "undefined") { // check for empty tiles
    ...
    var count = scene.tile_manager.tiles[coords].meshes.polygons.vertex_count;

    // use count instead of vbo.length because the vbo won't be the same size - it is resized in
    // chunks as needed, and might have old tile data in it
    // for (var i=0; i < vbo.length / stride; i++) {
    for (var i=0; i < count; i++) {
      // for every [stride] elements, copy the first three elements x, y, and z
      // adding the offset to the x and y to lay all the tiles out in the same world space
      // use the conversion factor so the building heights are to the same scale as the x & y
...
filenames = [];
function zipBlobs() {
  if (vbos.length == 0) { console.log("No files to zip!"); return; }
