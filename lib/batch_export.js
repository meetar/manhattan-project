...
function processVerts(coords, offset, name) {
  ...
  // 3rd try
  verts = new Int16Array(scene.tile_manager.tiles[coords].meshes.polygons.vertex_data.buffer);
  // Int16Array[191200]
  v = []
  // []
  // verts.length
  ...
  // var stride = scene.tile_manager.tiles["local/16/19294/24642/16"].meshes.polygons.vertex_layout.stride / 2;
  // undefined
  for (var i=0; i < verts.length / stride; i++) { v[i] = [verts[i*stride], verts[i*stride+1], verts[i*stride+2]].join(' '); }
    console.log(v);
  // [0, 0, 0]
